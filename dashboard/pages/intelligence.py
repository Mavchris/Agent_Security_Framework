...
st.markdown("""
    <div style='text-align: center; margin-bottom: 30px;'>
        <h1>📊  Veille Menaces sur Agent IA</h1>
        <p style='font-size: 16px; color: #666;'>Vue d'ensemble + Monitoring automatisé des menaces agents IA</p>
    </div>
...
with tab1:
    ...
    if stats:
        ...
        st.markdown("### ⚙️ État de l'Orchestrateur")
        ...
        if metrics:
            ...
            with col1:
                st.info(f"📅 Dernière exécution: {metrics.get('last_run_time', 'Jamais')}")
            
            with col2:
                st.info(f"🔄 Prochaine: Quotidien 02:00 UTC | Hebdo Lundi 10:00 UTC")
        else:
            st.warning("⚠️ Orchestrateur pas encore exécuté. Lancez: python orchestrator.py --test")
        
        st.divider()
        
        # ========== SECTION 3 : Charts Distributions ==========
        st.markdown("### 📈 Distributions des Menaces")
        ...
        # ========== SECTION 4 : Tendances ==========
        st.markdown("### 📊 Tendances et Analyse")
...
with tab2:
    ...
    if latest:
        ...
        st.info(f"📊 {len(filtered)} menaces affichées")
...
with tab3:
    st.subheader("⚙️ État de l'Orchestrateur")
    ...
    if metrics:
        ...
        with col1:
            st.metric("Exécutions Totales", metrics.get('total_runs', 0))
        
        with col2:
            st.metric("Réussies", metrics.get('successful_runs', 0))
        
        with col3:
            st.metric("Échouées", metrics.get('failed_runs', 0))
        ...
        st.warning("⚠️ Orchestrateur pas encore exécuté")
...
with tab4:
    st.subheader("📋 Logs de l'Orchestrateur")
